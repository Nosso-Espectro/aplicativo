# Plano de implementação — Assistente de IA

Spec: [spec.md](spec.md). Milestone #23; Issues #35, #36, #37. Plano proposto, sujeito às questões em aberto.
...
Módulo proposto: `src/features/assistant/`.

- `src/features/assistant/task-suggestions` — [US-035] Decompor tarefa com IA.
- `src/features/assistant/message-rewrite` — [US-036] Simplificar ou reformular uma mensagem com IA.
- `src/features/assistant/assistant-boundaries` — [US-037] Aplicar limites de segurança ao assistente.

## Modelo de dados

Solicitação explícita (texto e ação), sugestão gerada editável, confirmação de aplicação e destino de salvamento a esclarecer. Credenciais de serviço não pertencem ao cliente.

Representação física, tipos e migrações serão definidos após a escolha técnica; não instalar banco por antecipação.

## Persistência

Somente aplicar/salvar com confirmação (#35–#36). Fornecedor, processamento local/remoto e retenção do serviço indefinidos; nenhum backend será introduzido no MVP.

## Fluxos principais

Sequência de capacidades: Decompor tarefa com IA → Simplificar ou reformular uma mensagem com IA → Aplicar limites de segurança ao assistente.
...
O assistente depende da definição do processamento e proteção de credenciais. Não embutir segredo de provedor no aplicativo nem introduzir um backend no MVP para viabilizar a milestone P3.
