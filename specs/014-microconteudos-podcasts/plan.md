# Plano de implementação — Microconteúdos / Podcasts

Spec: [spec.md](spec.md). Milestone #22; Issues #33, #34. Plano proposto, sujeito às questões em aberto.
...
Módulo proposto: `src/features/micro-podcasts/`.

- `src/features/micro-podcasts/episode-list` — [US-033] Listar microconteúdos em áudio.
- `src/features/micro-podcasts/episode-player` — [US-034] Reproduzir micro-podcast.

## Modelo de dados

Episódio (identificador, título, duração, descrição, origem do áudio); estado de reprodução/posição na sessão e indicação de reproduzido quando disponível.

Representação física, tipos e migrações serão definidos após a escolha técnica; não instalar banco por antecipação.

## Persistência

Posição preservada durante a sessão (#34). Cache, download, retenção e origem do catálogo em aberto.

## Fluxos principais

Sequência de capacidades: Listar microconteúdos em áudio → Reproduzir micro-podcast.
